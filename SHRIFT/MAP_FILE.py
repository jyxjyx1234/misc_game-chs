...
database_keys = ["@name", "@nickname", "@description", "@displayName", "@message1", "@message2"]
...
class PAGE_LIST_CONTENT:
    ...
    def process_108(self):
        path = self.path.copy()
        path.append("@parameters")
        res = []
        for i, t in enumerate(self.parameters):
            if type(t) == str:
                res = [{"ori" : t, "path" : path + [i], "code" : self.code}]
        return res
...
class MAP_FILE:
    def __init__(self, file_path): # 只关注events里的pages里的list
        ...
        if "@events" not in self.data:
            return
        path.append("@events")
        if type(self.data["@events"]) == list:
            path.append(-1)
        for event in self.data["@events"]:
            if type(self.data["@events"]) == dict:
                path.append(event)
                event = self.data["@events"][event]
            else:
                path[-1] += 1
            if type(event) != dict:
                continue
