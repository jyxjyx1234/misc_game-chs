...
def pack():
	src = open('TEXT.DAT', 'rb')
	fin = open('text_trans.txt', 'r',encoding='utf16')
	dst = open('release/data/TEXT.DAT', 'wb')
	fp = open('SCRIPT.SRC','rb')
	ofp = open('release/data/SCRIPT.SRC','wb')
	...
	for rows in fin:
		...
		try:
			if int(row[0]) >= 0x315:
				...
				ofp.write(to_bytes(dst.tell(), 4))
				num = to_bytes(int(row[0]), 4)
				dst.write(num)
				dst.write(row[1].encode("cp932"))
				'''
				if dicts[ch] > 0xFF:
