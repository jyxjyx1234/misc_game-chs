...
DataManager.loadDataFile = function(name, src) {
    ...
    // var url = 'data/' + src;
    var url = window.selectedLanguage + "/" + src;
    xhr.open('GET', url);
...
DataManager.loadMapData = function(mapId) {
    if (mapId > 0) {
        ...
        this._mapLoader = ResourceHandler.createLoader(window.selectedLanguage + "/" + filename, this.loadDataFile.bind(this, '$dataMap', filename));
    ...
    if (globalInfo && globalInfo[savefileId]) {
        if (StorageManager.isLocalMode()) {
            ...
            return (savefile.globalId === this._globalId &&
                    savefile.title === "メルフィアス 蒼紅のヴァージェ ver1.223");
        }
    } else {
        return false;
    }
...
DataManager.makeSavefileInfo = function() {
    ...
    info.title      = "メルフィアス 蒼紅のヴァージェ ver1.223";
