...
class YSTB_command:
    ...
    def type(self) -> str:
        ...
        #    return "sound"
            ...
            return "may_be_opt"
        elif self.opcode == b'\x22\x00\x03\x00' or self.opcode == b'\x21\x00\x03\x00':
            #print(self.opcode)
            return "name_def"
        else:
            return "others"
...
class YSTB_FILE:
    ...
    def dump_text_to_file(self, path):
        ...
        for command in self.command_list:
            ...
            outputfile.write(f'##[{command_offset}]\n##{command.opcode.hex()} {hex(command.read_len)} {hex(command.content_offset)}\n')
            if command_type == "text":
                data = self._read_bytes_from_command(command)
                if data and data[0:2] != b'H\x03' and b"\x00" not in data and b"cg" not in data:
                    try:
            ...
            if command.opcode[-2:] == b"\x00\x00" and command.type() != "text" and textflag:
                outputfile.write(f"#[textend]\n")
                textidx += 1
                textflag = False
