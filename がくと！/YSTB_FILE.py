...
class YSTB_command:
    ...
    def read_command_bytes(self, data):
        ...
        self.read_len = from_bytes(data[8:12])
        self.content_offset = from_bytes(data[12:16])
    ...
    def type(self) -> str:
        ...
        #    return "sound"
            ...
            return "may_be_opt"
        elif self.opcode == b'\x12\x00\x03\x00' or self.opcode == b'\x13\x00\x03\x00':
            #print(self.opcode)
            return "name_def"
        else:
            return "others"
...
class YSTB_command_v2(YSTB_command):
    ...
    top_offset: int
    def type(self) -> str:
        if self.opcode == b'\x00\x00\x00\x00\x00\x00\x00\x00':
            return "text"
        if self.opcode == b'\x00\x00\x00\x00\x03\x00\x00\x00':
            return "name_def"
        else:
            return "others"
...
class YSTB_FILE:
    ...
    def _read_command(self) -> list:
        ...
        while p < self.command_len:
            command = YSTB_command(bytes(self.commands[p : p + 16]))
    ...
    def append_trans(self,command_offset:int,trans:str):
        ...
        self.commands[command_offset+8:command_offset+16] = list(to_bytes(length,4) + to_bytes(offset,4))
...
class YSTB_NAMEDEF_FILE(YSTB_FILE):
    ...
    def changename(self, isChangeCharset: bool = False, namedict = None):
        ...
        for command in self.command_list:
            ...
            if b"\x48\x24" in content_data:
                continue
            #print(command_offset)
            name = content_data[2:-2].decode(encoding='sjis')
            nameslist[name] = name
            if namedict:
            ...
            newdata = content_data[:2] + transname + content_data[-2:]
            print(newdata)
            self._append_new_namedef(command_offset, newdata)
        save_json("namedict.json", nameslist)
...
class YSTB_commandsdata_v2:
    ...
    def _read_command(self):
        ...
        command["content"] = []
        for i in range(len):
            c = YSTB_command_v2(data[6 + 16 * i:22 + 16 * i])
            c.command_offset = self.p + 6 + 16 * i
            c.top_op = op
            c.top_offset = self.p
            if op == b"\x4d" or op == b"\x22":
