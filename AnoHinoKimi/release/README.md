本补丁由 Steins;Gate、julixian、coroz、SUAD、ALyCE、是幼微鸭mua、冥语 等集资，使用官方渠道claude-3.5-sonnet进行翻译。
...
将补丁中的内容覆盖到游戏目录。从`AnoHinoKimi_CHS.exe`启动，无需转区。字体无需安装。免验证补丁已经集成，请勿再安装。

如果启动不成功（如提示缺少dll）请先尝试安装 https://aka.ms/vs/17/release/vc_redist.x64.exe 后重启。
