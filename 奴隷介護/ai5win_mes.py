...
class AI5WINScript(SilkyMesScript):
    ...
    command_library = {
        ...
        1: (
            ...
            (0x0e, 'VI', 'UNK_0E'),
            (0x0f, 'V', 'UNK_0F'),
            (0x10, '', 'UNK_10'),  # Need to recheck offset.
            # (0x11, 'V', 'INTERRUPT'),
            (0x11, '', 'INTERRUPT'),
            (0x12, 'CI', 'JUMP_IF2'),
            (0x13, '', 'UNK_13'),
            (0x14, 'C', 'UNK_14'),  # Need to recheck offset.
            (0x15, 'I', 'MENU'),
            (0x16, 'BC', 'FLAG_D_SET'),
        ),
        # Changes in commands...
    ...
    def assemble(self) -> None:
        ...
        # print(self._prm, self._first_offsets, self._offsets)
        # exit()
        if self._verbose:
            print("Parameters:", self._prm[0:1])
            if self.version >= self.header_version_threshold:
    ...
    def _assemble_offsets_and_parameters(self) -> tuple:
        ...
        while True:
            ...
            if line[1] == '0':  # "Free bytes".
                ...
                if self.command_library[self.version][command_index][0] == 0x15:  # Since header save offsets to messages.
                    message_count += 1
                ...
                try:
                    ...
                    print(line)
                    quit()
                if self.command_library[self.version][command_index][0] == 0x15:  # For this to not cause any errors.
                    argument_list[0] = 0
                argument_bytes = self.set_args(argument_list, self.command_library[self.version][command_index][1], self.encoding)
        ...
        while True:
            ...
            if lib_index != -1:
                if stringer != '':
                    ...
                    stringer = '#0-{}\n'.format(stringer)
                    print("warning: #0 at " + self._mes_name)
                ...
                if self.command_library[self.version][lib_index][0] == 0x15:
    ...
    def _diss_other_offsets(self) -> None:
        ...
        while True:
            ...
            current_byte = in_file.read(1)
            # print("current_byte", current_byte.hex() , "at", pointer)
            if current_byte == b'':
                break
            ...
            if lib_index != -1:
                if self._verbose:
                    print(hex(current_byte), pointer)
                ...
                what_index = -1
                # print("Current byte:", hex(current_byte))
                for entry_pos, offsets_entry in enumerate(self.offsets_library[self.version]):
                    if current_byte == offsets_entry[0]:
                        what_index = entry_pos
                # print("What index:", what_index)
                if what_index != -1:
                    not_here = True
    ...
    def get_C(file_in, current_byte: int, current_encoding: str) -> list:
        ...
        while True:
            ...
            # print("Struct byte:", struct_byte, "at", file_in.tell() - 1)
            struct_index = - 1
    ...
    def get_G(in_file, current_byte: int, current_encoding: str):
        ...
        while True:
            ...
            if continue_flag:
                if continue_flag != 1:
                    # print("Warning: unexpected continue flag in group structure: {}!".format(continue_flag))
                    grouper.append("###warning###")
                new_arg = AI5WINScript.get_args(in_file=in_file,
                                                args="C",
                                                current_byte=current_byte,
                                                current_encoding=current_encoding)
