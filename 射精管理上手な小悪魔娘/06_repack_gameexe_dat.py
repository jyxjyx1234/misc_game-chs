from LIB.Lib import *
from LIB.lzss import *
from LIB.PCK_FILE import key, xor

ori_gameexe_dat_path = "gameexe.ini"#修改后的gameexe.ini文件
output_path = "release\\GameexeZH.dat"#输出的gameexe.dat封包文件

# ================================================

key2 = []

data = open_file_b(ori_gameexe_dat_path)
data = data.decode("utf-16")
data = data.encode("utf-16le")
orisize = len(data)
data = compress(data)
newsize = len(data)
data = to_bytes(newsize + 8, 4) + to_bytes(orisize, 4) + data
data = xor(data, bytes(key2))
data = b"\x00" * 8 + data
save_file_b(output_path, data)