...
class PCK_Scene_Data:
    ...
    def compress(self, name=None):
        size = len(self.data)
        # ori = self.data
        if name:
            save_file_b("temp1/"+name, self.data)
        self.data = compress(self.data)
        # re = decompress(self.data)
        # if re != ori:
        #     print("Compress-Decompress check failed.")
        #     raise Exception("Compress-Decompress check failed.")
...
class PCK_File:
    ...
    def compress_and_enc_all_scenes(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            executor.map(compress_scene, self.scenes.keys())
