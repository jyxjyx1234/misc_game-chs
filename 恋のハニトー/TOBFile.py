...
class TOBText():
    NamePatten = re.compile(rb"(\x1A\x00\x00\x00\x01)(?P<alllen>[\x01-\xff])(\x00\x00\x00\x02\x01\x01)(?P<namelen>[\x00-\xff][\x00-\xff])(?P<name>[\x01-\xff]*?)(?=\x00)")
    ...
    def __init__(self, data, oriStart, oriEnd) -> None:
        self.data = data
    ...
    def readText(self):
        '''
        女主对话：\x1A\x00\x00\x00\x01\x1a\x00\x00\x00\x02 \x01\x01[2位长度][人名]\x00 \x01\x01[2位长度][声音文件]\x00 文本
        男主对话：\x1A\x00\x00\x00\x01\x0d\x00\x00\x00\x02 \x01\x01[2位长度][人名]\x00 \x00 文本

        旁白：几种模式？看不懂
        \x10\x00\x00\x00\x01\x05\x00\x00\x00\x00 
        \x14\x00\x00\x00\x01\x0B\x00\x00\x00\x01\x01\x00\x2C\x01\x00\x00
        \x0E\x00\x00\x00\x01\x05\x00\x00\x00\x00（接分行后的文本？）
        \x1D\x00\x00\x00\x01\x0E\x00\x00\x00\x04\x01\x00[\x00-\xff][\x00-\xff][\x00-\xff][\x00-\xff]\x00\x00\x00
        \x02\xFF\xFF\xFF\xFF\x00\x02\x01\x00\x00\x00\x00


        选项：
        \x0D\x00\x00\x00\x01[\x00-\xff]\x00\x00\x00\x0A\x01\x00\x00\x00\x00\x00
        后面全是：\x01\x01[2位长度] 文本（00结尾）
        '''
        ...
        for i in self.datas:
            ...
            if self.NamePatten.search(i):
                def _(match):
                    ...
                    oriAlllen = from_bytes(match.group("alllen"))
                    deltaLen = len(transName) - len(match.group("name"))
                    newAlllen = to_bytes(oriAlllen + deltaLen, 1)
                    return b"\x1A\x00\x00\x00\x01" + newAlllen + b"\x00\x00\x00\x02\x01\x01" + transNameLen + transName
                transed = self.NamePatten.sub(_, transed)
                ...
                transed = self.selectionTextPatten.sub(_, transed)
            ...
            commandLen = from_bytes(transed[5:9])
            text = transed[5 + commandLen : ]
            if text and text[0] != 0x5b: 
                transed = transed[ : 5 + commandLen] + transBytesList.pop(0)
...
class TOBFile():
    ...
    def _readTexts(self):
        ...
        for i in range(self.offsetPart.count):
            ...
            self.textsList.append(TOBText(self.texts[start:end], start, end))
    ...
    def save(self, outPath):
        ...
        jumpPatten2 = re.compile(rb"(?<=\x5B\x20\x00\x00\x00\x00\x02\x3D\x01\x00\x00\x00\x00\x00\x00\x0B\x00\x00\x00\x01\x01\x00)([\x00-\xff]{4})")
