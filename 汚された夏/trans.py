        ...
        if match.group("msg"):
            ...
            if oriname:
                if oriname in namedict:
                    ...
                    transtext = transdatas.pop(0)["message"].replace("...", "…").replace("..", "…").replace("‥", "…").replace(".", "。").replace("⋯", "…").replace("･", "·").replace("・", "·").replace("･･･", "…").replace("⋯", "…")
                    ori_data = ori_data.replace(f'name="{oriname}"', f'name=""')
                    sp_text_flag = transtext
            msg = match.group("msg")
            if msg:
                transtext = transdatas.pop(0)["message"].replace("...", "…").replace("..", "…").replace("‥", "…").replace(".", "。").replace("⋯", "…").replace("･", "·").replace("・", "·").replace("･･･", "…").replace("⋯", "…")
                if sp_text_flag:
                    transtext = sp_text_flag + transtext
                    sp_text_flag = None
