...
import zipfile
zip_filename = "release\\eroyoubi.tpatch"

def zip_dir(zipf, folder_to_zip, zip_filename):
    for root, _, files in os.walk(folder_to_zip):
        for file in files:
            file_path = os.path.join(root, file)
            arcname = os.path.relpath(file_path, os.path.dirname(folder_to_zip))
            zipf.write(file_path, arcname)
def zip_file(zipf, path_to_file, zip_filename):
    zipf.write(path_to_file, os.path.basename(path_to_file))
zipf = zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED)
zip_dir(zipf, "release\\data", zip_filename)
zip_dir(zipf, "release\\tyrano", zip_filename)
zipf.close()
