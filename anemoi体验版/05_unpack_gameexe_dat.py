...
from LIB.PCK_FILE import key, xor

ori_gameexe_dat_path = "gameexe.dat"#原始的gameexe.dat文件
output_path = "gameexe.ini"#输出的gameexe.ini文件


# ================================================


key2 = [0xD8,0x29,0xB9,0x16,0x3D,0x1A,0x76,0xD0,0x87,0x9B,0x2D,0x0C,0x7B,0xD1,0xA9,0x19,
...
if __name__ == "__main__":
    data = BytesReader(open_file_b(ori_gameexe_dat_path))
    unk = data.readU32()
    isenc = data.readU32()
    content = data.read()
    if isenc == 1:
        content = xor(content, key)
    isenc = 0
    content = xor(content, bytes(key2))
    content = decompress(content[8:])
    content = content.decode("utf-16le")
    content = content.encode("utf-16")
    save_file_b(output_path, content)