from Lib import *
import re

def processSlet(text:str) -> list[str]:
    return re.findall(r"(?<=sel[0-9]=')(.*?)(?=')", text)

def splitmsg(text:str) -> str:
    maxLineLen = 24
    res = []
    while len(text) > maxLineLen:
        res.append(text[:24])
        text = text[24:]
    res.append(text)
    return "".join(res)

def remove_nested_brackets(text, ifPrint = False):
    oriText = text
    while True:
        match = re.search(r'\[[^\[\]]*\]', text)
        if match:
            if match.group() != "[p2]" and ifPrint:
                print(match.group())
                print(oriText)
            text = text.replace(match.group(), '')
        else:
            break
    text = re.sub(r"[「　」 \t]" , "", text)
    return text

name_patten = re.compile(r'(?<=\[setname. name=)(.*?)(?=\])')

class Line():
    def __init__(self, text:str, ifScr:bool = False) -> None:
        self.text = text
        self.type = ""
        self.msg = ""
        self.scr_flag = ifScr
        self.read()
    
    def read(self):
        if re.match(r"[\*;@]", self.text):
            self.type = 'others'
        elif name_patten.search(self.text):
            self.type = "name"
            self.msg = name_patten.search(self.text).group()
        elif remove_nested_brackets(self.text) == "":
            self.type = 'others'
        else:
            self.type = "msg"
            self.msg = remove_nested_brackets(self.text, ifPrint = True)

    def trans(self, namedict, transdict):
        match self.type:
            case "name":
                self.text = name_patten.sub(lambda x: namedict[x.group()], self.text)
            case "msg":
                self.text = transdict[remove_nested_brackets(self.text)] + "[p2]"

class KS_FILE():
    def __init__(self, path) -> None:
        data = open_file_b(path)
        lines = data.split(b'\x0d\x0a')
        self.lines = []
        ifScr = False
        for i in lines:
            l = Line(i.decode("cp932"), ifScr)
            self.lines.append(l)
            if l.type == "scr_start":
                ifScr = True
            if l.type == "scr_end":
                ifScr = False

    
    def dump_text(self) -> OriJsonOutput:
        out = OriJsonOutput()
        for l in self.lines:
            if l.type == "name":
                out.dic['name'] = l.msg
            if l.type == "msg":
                out.dic['message'] = l.msg
                out.append_dict()
        return out

    def trans(self,namedict,transdict):
        i = 0
        while i < len(self.lines):
            l = self.lines[i]
            if l.type == "msg" or l.type == "name":
                l.trans(namedict, transdict)
            i += 1
    
    def save(self, path):
        f = open(path,'w',encoding='utf16')
        for l in self.lines:
            f.write(l.text)
            f.write("\n")
