...
def replace_halfwidth_with_fullwidth(string):
    # 将半角符号替换为全角符号
    halfwidth_chars = "!\"#$&'()*+,-./:;<=>?@[\\]^_`{|}~abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
    fullwidth_chars = "！＂＃＄＆＇（）＊＋，－．／：；＜＝＞？＠［＼］＾＿｀｛｜｝～ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ０１２３４５６７８９"
...
class OriJsonOutput():
    def __init__(self) -> None:
        self.savefilter = lambda x: "message" in x
        self.textcount = 0
        self.messageset = set()
        self.outlist = []
        self.dic = {}
