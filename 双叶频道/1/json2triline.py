...
#h.ChangeFont("wenquanyi.ttf", "release\\futaba1.ttf", "futaba1")
for n in namedict:
...
for filename in filelist:
    ...
    for i in range(len(ori_triline)):
        if ori_triline[i][:4] != "TR1=":
            ...
            if ori_triline[i].endswith("："):
                out_triline.write("TR1=" + namedict[ori_triline[i]] + "\n")
            elif "：" in ori_triline[i]:
                trans = trans_json.pop(0)["message"]
                trans = preProcess(trans)
                name,message = ori_triline[i].split("：",1)
                name = namedict[name + "："]
                out_triline.write("TR1=" + name + trans + "\n")
            else:
                trans = trans_json.pop(0)["message"]
                trans = preProcess(trans)
                out_triline.write("TR1=" + trans + "\n")
    out_triline.close()  
...
for filename in filelist:
    name_def_f = YSTB_NAMEDEF_FILE_v2(path=ori_path+filename,encrypt=encrypt)
