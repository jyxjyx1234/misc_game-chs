...
def draw_char(sizeimg, char, font, color):
    img = Image.new("L", (sizeimg, sizeimg), 0)
    # font = ImageFont.truetype("alyce_humming.ttf", int(sizeimg * 0.9))
    draw = ImageDraw.Draw(img)
    draw.text((0, 0), char, font=font, fill=color)
    data1 = img.tobytes()
    img = Image.new("L", (sizeimg, sizeimg), 0)
    draw = ImageDraw.Draw(img)
    draw.text((0, 0), char, font=font, fill=16, stroke_fill=16, stroke_width=1)
    data2 = img.tobytes()
    return data1, data2
...
class HanziReplacer(object):
        ...
        for char in text:
            try:
                ...
                if len(char.encode("sjis")) != 2:
                    raise Exception(f'输入文本中有不支持的字符{char}，请检查你的文本')
    ...
    def gen_FNT(self, oriCharList, outTBLs, outFNTs):
        # 生成TBL（标称字表）
        # charlist = oriCharList + self.source_chars + self.allChars
        charlist = self.source_chars
        app_list = []
        for char in self.allChars:
            if char not in charlist:
                app_list.append(char)
        charlist += "".join(app_list)
        
        true_TBL = self.target_chars
        true_TBL += "".join(app_list)
            
        # print(true_TBL)
        TBL = b""
        TBL += to_bytes(len(charlist), 2)
        for char in charlist:
            TBL += char.encode('932')
        TBL += b'\x00\x00'
        for outTBL, sp in outTBLs:
            if not sp:
                os.makedirs(os.path.dirname(outTBL), exist_ok=True)
                save_file_b(outTBL, TBL)
            else:
                sp_TBL = b""
                sp_TBL += to_bytes(len(sp), 2)
                for char in sp:
                    sp_TBL += self.hanzitihuan(char).encode('932')
                sp_TBL += b'\x00\x00'
                os.makedirs(os.path.dirname(outTBL), exist_ok=True)
                save_file_b(outTBL, sp_TBL)
        # 生成FNT（字体文件）和MSK（掩码文件）
        # outFNTs: [{"outpath": "", "font_size": xx}]
        for outFNT in outFNTs:
            outpath = outFNT["outpath"]
            os.makedirs(os.path.dirname(outpath), exist_ok=True)
            color = outFNT["FNT_MAX"]
            SP_CHAR_RANGE = outFNT.get("SP_CHAR_RANGE", None)
            with open(outpath + ".FNT", 'wb') as FNTF:
                with open(outpath + ".MSK", 'wb') as MSKF:
                    font_size = outFNT["font_size"]
                    font = ImageFont.truetype("alyce_humming.ttf", int(font_size * 0.9))
                    if not SP_CHAR_RANGE:
                        for char in true_TBL:
                            data1, data2 = draw_char(font_size, char, font, color)
                            FNTF.write(data1)
                            MSKF.write(data2)
                    else:
                        for char in SP_CHAR_RANGE:
                            data1, data2 = draw_char(font_size, char, font, color)
                            FNTF.write(data1)
                            MSKF.write(data2)
