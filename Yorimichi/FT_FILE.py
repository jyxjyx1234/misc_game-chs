...
fonts = [0] + [ImageFont.truetype("攸望POP3体W12（简繁）.ttf", i) for i in range(1, 30)]

font_add = [0] + [ImageFont.truetype("msyh.ttc", i) for i in range(1, 30)]
...
def main():   
    ...
    data = open_file_b("font\\text.ft")
    ft = FT_FILE(data)
    ft.rebuild(replace_dict)
    save_file_b("release\\trans\\text.ft", ft.to_bytes())
    data = open_file_b("font\\name.ft")
    ft = FT_FILE(data)
    ft.rebuild(replace_dict)
    save_file_b("release\\trans\\name.ft", ft.to_bytes())
