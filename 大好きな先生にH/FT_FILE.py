...
fonts = [0] + [ImageFont.truetype("FZY1JW.TTF", i) for i in range(1, 50)]
...
def main():   
    ...
    for file in os.listdir("font"):
        ...
        save_file_b(f"release\\trans\\{file}", ft.to_bytes())
        if file == "DEFAULT.FT":
            ft.show("叮")
    save_json("not_found.json", not_found)

if __name__ == "__main__":
    ft = FT_FILE(open_file_b("font\\DEFAULT.FT"))
    ft.show("ピ")