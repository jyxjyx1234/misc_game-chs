...
class BCS_FILE:
    ...
    def dump(self, out: OriJsonOutput):
        ...
        for i, line in enumerate(self.lines):
            if len(line) < 0x0e or i == 0:
                continue
            if line[1].startswith("$menu "):
                cmds = line[1].split(",")
                out.add_text(cmds[0][6:])
                out.dic["pos"] = f"{i} 1 menu"
                out.append_dict()
            if line[9]:
                out.add_name(line[9])
                out.dic["name_pos"] = f"{i} 9 name"
            if line[13]:
                out.add_text(line[13])
                out.dic["pos"] = f"{i} 13 text"
                out.append_dict()
    ...
    def trans(self, transdata, namedict, h: HanziReplacer):
        for trans in transdata:
            ...
            if type == "menu":
                ori = self.lines[l][1].split(",")
                ori[0] = "$menu " + transtext
                self.lines[l][1] = ",".join(ori)
            else:
