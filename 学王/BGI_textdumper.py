    ...
    def dump(self, outpath):
        ...
        while self.file.p < first_offset and self.file.p < len(self.filedata):
            ...
            if op == 3:
                ...
                if offset + self.headerALLlen < first_offset:
                    self.file.p -= 4
                    continue
                # if length == 0:
                #     continue
                ...
                elif re.match(rb"\x3F\x00\x00\x00[\x00-\xff]{4}\x43\x01\x00\x00", self.file.tryRead(12)):
                    out.write(f"[spmsg]<{self.file.p - 4 - self.headerALLlen},{offset},{length}>{text}\n")
                elif re.match(rb"\x3F\x00\x00\x00[\x00-\xff]{4}\x18\x01\x00\x00", self.file.tryRead(12)):
                    out.write(f"[titlemsg]<{self.file.p - 4 - self.headerALLlen},{offset},{length}>{text}\n")
                elif re.match(rb"\x09\x00\x00\x00\x02\x00\x00\x00\x7f\x00\x00\x00", self.file.tryRead(12)) and "select" in self.path:
                        out.write(f"[optmsg]<{self.file.p - 4 - self.headerALLlen},{offset},{length}>{text}\n")
                elif re.match(rb"\x03\x00\x00\x00[\x00-\xff]{4}\x1c\x00\x00\x00", self.file.tryRead(12)) and read_until_null(self.filedata, from_bytes(self.file.tryRead(8)[4:]) + self.headerALLlen) == b"_SubMessage":
                        out.write(f"[submsg]<{self.file.p - 4 - self.headerALLlen},{offset},{length}>{text}\n")
                else:
