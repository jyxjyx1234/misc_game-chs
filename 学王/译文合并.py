    ...
    for line in file:
        ...
        if filter(content):
            if type == "ruby1" or type == "ruby2":
                content = transdict.get(content, content)
                content = namelist.get(content, content)
                content = notfound_trans.get(content, content)
        ...
        if content.startswith("u8_"):
            ...
            # content = h.hanzitihuan(content).encode("utf-8")
            content = content.encode("utf-8")
