...
#z01
	...
	while (1)	{
		...
		if (@btn_save <= l[2] && l[2] < @btn_save + @save_cnt)	{
			if(@確認チェック == 0 && syscom.get_save_exist($save_page_no * 10 + l[2] - @btn_save) == 1){
				l[1] = $save_alert(0)	// セーブ確認
				if(l[1] == 0){
					syscom.save($save_page_no * 10 + l[2] - @btn_save, 0, 1)
					$update_save_obj(-1)
				}
			}
			else{
...
#init
	...
	// セーブタイトル
	excall.back.object[@obj_title].create(_saveload_title, 1, 0, 25, 0)
	...
	// =============================================================
	// チェック
	excall.back.object[@btn_check].create(_saveload_check, 1, 521, 575, @確認チェック)
	excall.back.object[@btn_check].set_button(@btn_check, 0, 0, 0)
...
//	if($check == 0){
		excall.front.object[@btn_check].create(_saveload_check, 1, 521, 575, @確認チェック)
