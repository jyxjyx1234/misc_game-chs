...
def from_bytes(b:bytes, signed = False)->int:
...
def to_bytes(num:int,length:int, signed=False)->bytes:
    return num.to_bytes(length,byteorder='little',signed=signed)
...
class OriJsonOutput():
    ...
    def add_text_idx(self, idx):
        self.dic["text_idx"] = self.dic.get("text_idx",[]) + [idx]
    
    def add_name_idx(self, idx):
        if "name_idx" in self.dic:
            raise Exception("name_idx already exists")
