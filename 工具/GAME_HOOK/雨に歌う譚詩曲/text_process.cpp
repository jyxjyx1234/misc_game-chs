...
void dump_text() {
    ...
    file.write((char*)0x54B740, fileLen);
    file.close();
}
...
void replace_file(char** filename) {
	...
	if (!CustomPack::isInPack("AME_CHS.CPK", *filename)) {
		//MessageBoxA(NULL, (std::string("Unable to open file ") + *filename).c_str(), "Error", MB_OK);
		return;
	}
	//std::string content((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
	std::string content = CustomPack::getFile("AME_CHS.CPK", enc, *filename);
	...
	memcpy((char*)0x54B740, content.c_str(), content.size());
}
...
void InstallHook_replacetext()
{
	...
	if (readFilenamesFromFile()) {
        ...
        *(DWORD*)(originalFuncAddr + 1) = (DWORD)HookFunction_dump_getFileLen - originalFuncAddr - 5;
        ...
        originalFuncAddr = 0x0040B197;
        returnAddress3 = 0x0040AFD0;
        VirtualProtect((LPVOID)originalFuncAddr, 5, PAGE_EXECUTE_READWRITE, &oldProtect);
        *(BYTE*)originalFuncAddr = 0xE9;
    ...
    *(DWORD*)(originalFuncAddr + 1) = (DWORD)HookFunction_replace_replaceOffset - originalFuncAddr - 5;
    ...
    install_hook_textreplaceFromPackEx(1, "AME_CHS.CPK","data2.bin", enc);
