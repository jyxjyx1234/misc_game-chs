...
void replace_text(char** text1, char** text2) {//普通文本
	if (*text1 != nullptr) {
	    ...
	    if (it != replacementMap.end()) {
		    int idx = it->second;
		    strcpy_s(*text1, strlen(strBuffer[idx]) + 1, strBuffer[idx]);
	    }
	}
    if (*text2 != nullptr) {
        ...
        if (it != replacementMap.end()) {
            int idx = it->second;
            strcpy_s(*text2, strlen(strBuffer[idx]) + 1, strBuffer[idx]);
        }
    }
...
void InstallHook_replacetext()
{
    ...
    //普通文本
    DWORD oldProtect;
    originalFuncAddr = baseAddress + 0xa1bd0;
    returnAddress = originalFuncAddr + 8;
