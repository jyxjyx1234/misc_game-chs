...
std::map<std::string, int> readKeyValuePairsFromFile(const std::string& filename, std::string k) {
    ...
    std::string transData = CustomPack::getFile("KimiIta_CHS.cpk", k, filename);
...
void replace_text(char** text) {
	...
	if (it != replacementMap.end()) {
		isNeedRepair = true;
		oriLen = strlen(strBuffer[it->second].c_str()) + 1;
		oriPos = *text;
		memcpy(oriText, *text, oriLen);
		memcpy(*text, strBuffer[it->second].c_str(), oriLen);
		(*text)[oriLen - 1] = '\0';
	}
    else {
        isNeedRepair = false;
...
void repair_text() {
	if (!isNeedRepair) return;
	if (oriText == nullptr || oriPos == nullptr) return;
	if (oriLen <= 0) return;
	memcpy(oriPos, oriText, oriLen);
}
