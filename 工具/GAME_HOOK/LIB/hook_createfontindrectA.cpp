...
HFONT WINAPI HookedCreateFontIndirectA_changefont(CONST LOGFONTA* lplf)
{
    ...
    printf("Change Font From: %s To: %s\n", modifiedLf.lfFaceName, fontn.c_str());
...
std::string WCharTo932(const wchar_t* wstr)
{
    int bufferSize = WideCharToMultiByte(932, 0, wstr, -1, NULL, 0, NULL, NULL);
    if (bufferSize == 0) {
        return "";
    }

    std::string result(bufferSize, 0);
    WideCharToMultiByte(932, 0, wstr, -1, &result[0], bufferSize, NULL, NULL);
