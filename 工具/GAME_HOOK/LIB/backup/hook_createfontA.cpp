...
bool ifchange(
    ...
    if ((cHeight != targetCHEIGHT && cHeight != targetCHEIGHT2 && cHeight != targetCHEIGHT3 && targetCHEIGHT != -1)) {
        return false;
    }
    if ((cWeight != targetCWEIGHT && targetCWEIGHT != -1)) {
        return false;
    }    
    if ((cWidth != targetCWIDTH && targetCWIDTH != -1)) {
        return false;
    }    
    if ((pszFaceName_ != targetFONTNAME && targetFONTNAME != "-1") || pszFaceName_ == nFONTNAME) {
        return false;
    }
    if (!((cWidth != nCWIDTH || nCWIDTH == -1) && (cWeight != nCWEIGHT || nCWEIGHT == -1) && (cHeight != nCWEIGHT || nCWEIGHT == -1))){
        return false;
    }
	return true;
}
