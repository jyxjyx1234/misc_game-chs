...
// {D1D1D975-BB9F-4e93-AAC3-1E69DE38A9D7}
DEFINE_GUID(IID_IMiniportAudioEngine, 
0xd1d1d975, 0xbb9f, 0x4e93, 0xaa, 0xc3, 0x1e, 0x69, 0xde, 0x38, 0xa9, 0xd7);
...
DECLARE_INTERFACE_(IMiniportAudioEngine,IUnknown)
{
    DEFINE_ABSTRACT_UNKNOWN()   //  For IUnknown
    ...
    STDMETHOD_(NTSTATUS,GetProcessingPeriod)
    (   THIS_
        _In_  ULONG ulNodeId,
        _Out_  PLONGLONG pllProcessingPeriod
    )   PURE;
...
#define IMP_IMiniportAudioEngine                    \
    ...
    STDMETHODIMP_(NTSTATUS) GetProcessingPeriod  \
    (       \
        _In_  ULONG ulNodeId,\
        _Out_ PLONGLONG pllProcessingPeriod\
    );\
 ...
 *****************************************************************************
 * GetProcessingPeriod() has been removed from the IMiniportAudioEngineNode interface
 * Any existing miniport driver using IMiniportAudioEngineNode would have to remove 
 * its GetProcessingPeriod() implementation to avoid compilation error when compiled 
 * with this version portcls.h
 */
