...
#if defined(_M_IX86) || defined(_M_IA64) || defined(_M_ARM)
...
#endif // defined(_M_IX86) || defined(_M_IA64) || defined(_M_ARM)
...
#if (_MSC_VER >= 1600) || defined(_PREFIX_) || defined(_PREFAST_)
...
//
// This protection value can only be used to specify to VirtualProtect
// that the argument range should be reverted back to point back to the
// backing file.  This means the contents of any private (copy on write)
// pages in the range will be discarded, and any pages whose protections
// are not the same as when the region was initially created will also
// be reverted.  Any reverted pages that were locked into the working set 
// are unlocked as well.
//
// When this protection value is used, the lower protection bits must
// be the same as the ones used to initially create the region.
//
...
#if defined(_AMD64_) || defined(_ARM_) || defined(_IA64_)
...
#endif // defined(_AMD64_) || defined(_ARM_) || defined(_IA64_)
...
RtlExtractBitMap (
    ...
    _In_range_(0, Source->SizeOfBitMap - 1) ULONG NumberOfBits
    );
...
#if (defined(_M_IX86) && (_MSC_FULL_VER > 13009037)) || ((defined(_M_AMD64) || defined(_M_IA64)) && (_MSC_FULL_VER > 13009175)) || defined(_M_ARM)
#ifdef __cplusplus
extern "C" {
#endif
...
#define InterlockedExchangePointerAcquire InterlockedExchangePointer
...
#define InterlockedCompareExchangePointerNoFence InterlockedCompareExchangePointer
...
#define KeGetCurrentProcessorIndex() (*(ULONG const*)0xE0000000FFFF0B2C)
...
_At_(*FloatingState, _Kernel_requires_resource_not_held_(FloatState) _Kernel_acquires_resource_(FloatState))
__forceinline
NTSTATUS
KeSaveFloatingPointState (
...
_Success_(1)
_Kernel_float_restored_
_At_(*Floatingstate, _Kernel_requires_resource_held_(FloatState) _Kernel_releases_resource_(Floatstate))
__forceinline
NTSTATUS
KeRestoreFloatingPointState (
    _In_ PVOID FloatingState
    )
...
UCHAR
READ_PORT_UCHAR (
    _Notliteral_ PUCHAR RegisterAddress
    );

NTHALAPI
USHORT
READ_PORT_USHORT (
    _Notliteral_ PUSHORT RegisterAddress
    );

NTHALAPI
ULONG
READ_PORT_ULONG (
    _Notliteral_ PULONG RegisterAddress
    );

NTHALAPI
VOID
READ_PORT_BUFFER_UCHAR (
    _Notliteral_ PUCHAR portAddress,
    PUCHAR readBuffer,
    ULONG  readCount
    );
...
VOID
READ_PORT_BUFFER_USHORT (
    _Notliteral_ PUSHORT portAddress,
    PUSHORT readBuffer,
    ULONG  readCount
    );
...
VOID
READ_PORT_BUFFER_ULONG (
    _Notliteral_ PULONG portAddress,
    PULONG readBuffer,
    ULONG  readCount
    );
...
#if !(defined(_X86_) || defined(_AMD64_) || defined(_ARM_) || defined(_IA64_))
...
//
// These functions are imported for IA64, ARM, ntddk, ntifs, nthal, ntosp, and wdm.
...
#elif defined(_ARM_) || defined(_IA64_)
...
#if (!defined(_IA64_) && !defined(_X86_) && !defined(_AMD64_) && !defined(_NTOS_)) || defined(_PREFAST_)
...
#endif // !defined(_IA64_) && !defined(_X86_) && !defined(_AMD64_) && !defined(_NTOSP_)
...
#define PCI_EXPRESS_THUNDERBOLT_CAP_ID                                  0x001F
...
//
typedef union _PCI_EXPRESS_ACS_CAPABIITY_REGISTER {
...
} PCI_EXPRESS_ACS_CAPABIITY_REGISTER, *PPCI_EXPRESS_ACS_CAPABIITY_REGISTER;
...
typedef struct _PCI_EXPRESS_ACS_CAPABILITY {
    ...
    PCI_EXPRESS_ACS_CAPABIITY_REGISTER      Capability;
    ...
    )
{
    ...
    if (RuntimeFlags & DrvRtPoolNxOptIn)
    {
        //
