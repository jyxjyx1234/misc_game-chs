...
// currently (XCDB)
//
#define SRBEX_DATA_SCSI_CDB_VAR_LENGTH_MAX ULONG_MAX

typedef struct SRB_ALIGN _SRBEX_DATA_SCSI_CDB_VAR {
    _Field_range_(SrbExDataScsiCdbVar, SrbExDataTypeScsiCdbVar)
    SRBEXDATATYPE Type;
...
#define SRBEX_DATA_IO_INFO_LENGTH ((3 * sizeof(ULONG)) + (4 * sizeof(UCHAR)))
...
//
// Data set management IOCTL to match DSM notifications
//
...
////////////////////////////////////////////////////////////////////////////////
//
// Type definitions - SCSI Enclosure Services
//
...
#define SES_STATUS_NOT_INSTALLED                0x05
#define SES_STATUS_UNKNOWN                      0x06
#define SES_STATUS_NOT_AVAILABLE                0x07
#define SES_STATUS_NO_ACCESS                    0x08
...
typedef struct _SES_CONTROL_DESCRIPTOR {
    ...
    UCHAR Reserved : 4;                         // Byte 0, bits 0-3
    ...
    //
    // The below fields are specific
    // to a device slot element
    //
...
typedef struct _SES_PHY_DESCRIPTOR {
    ...
    UCHAR Reserved1 : 4;                        // Byte 0, bits 0-3
    UCHAR DeviceType : 3;                       // Byte 0, bits 4-6
...
typedef struct _SES_SAS_SLOT_INFORMATION {
    ...
    UCHAR Reserved1 : 5;                        // Byte 1, bits 1-5
    UCHAR Type : 2;                             // Byte 1, bits 6-7
...
typedef struct _SES_ADDITIONAL_ELEMENT_STATUS_DESCRIPTOR {
    ...
    UCHAR  ProtocolIdentifier : 4;              // Byte 0, bits 0-3
    UCHAR  EIP : 1;                             // Byte 0, bit 4
    UCHAR  Reserved1 : 2;                       // Byte 0, bits 5-6
...
#define STOR_MAP_NO_BUFFERS                         (0)     // only maps buffer for SRB_FUNCTION_IO_CONTROL and SRB_FUNCTION_WMI
...
#define REQUEST_INFO_NO_CACHE_FLAG          0x1
#define REQUEST_INFO_PAGING_IO_FLAG         0x2
#define REQUEST_INFO_SEQUENTIAL_IO_FLAG     0x4
#define REQUEST_INFO_TEMPORARY_FLAG         0x8
#define REQUEST_INFO_WRITE_THROUGH_FLAG     0x10
...
//
// A miniport must only specify one F-state (F0) for an adapter.
//
#define STOR_POFX_ADAPTER_MIN_IDLE_STATES 1
#define STOR_POFX_ADAPTER_MAX_IDLE_STATES 1
...
//  to active if device was put into either the idle or power off state when idle.
...
#endif // (NTDDI_VERSION >= NTDDI_WIN8) for Runtime Power Management
