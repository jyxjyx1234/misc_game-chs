...
class BCS_FILE:
    def __init__(self, path):
        ...
        self.line_len = self.data.readU32()
    ...
    def dump(self, out: OriJsonOutput):
        # 256700 ,,,,,,,,隆行,隆行,,かなみ,10101,「自信なさげだなぁ。ほんとに大丈夫？」,
        # 2100,"$menu ポコ,sr_poko",,,,,,,,,,,,
        out.preProcess = preprocess
        for i, line in enumerate(self.lines):
            if len(line) < 0x0e or i == 0:
                continue
            if line[1].startswith("$menu_item "):
                cmds = line[1].split(",") # 示例：在行内第1项以$menu开头时为选项
                out.add_text(cmds[1]) # 第一项中用逗号进行分割，第0项为选项内容
                out.dic["pos"] = f"{i} 1 menu"
                out.append_dict()
            if line[8]: # 示例：在行内第9项有值时为角色名
                out.add_name(line[8])
                out.dic["name_pos"] = f"{i} 8 name"
            if line[13]:# 示例：在行内第13项有值时为文本
                out.add_text(line[17])
                out.dic["pos"] = f"{i} 17 text"
                out.append_dict()
    ...
    def trans(self, transdata, namedict, h: HanziReplacer):
        for trans in transdata:
            ...
            idx = int(idx)
            l = int(l)
            transtext = trans["message"].replace("...", "…")
            ...
            if type == "menu":
                ...
                ori[1] = transtext # 这里修改导入的选项文本格式（这里是"$menu ポコ,sr_poko"，逗号分割的第0项目为选项文本）
...
if __name__ == "__main__":
    ...
    for file in os.listdir("scr_dec"):
        ...
        bcs.to_csv("scr_csv/" + file.lower().replace(".bcs", ".csv"))
    for op in unkop:
        print(hex(op))