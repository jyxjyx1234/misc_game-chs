...
for dic in yiwen:
    s=dic["pre_jp"].split('。')
    st=dic["pre_zh"].split('。')
    if len(s)==len(st) and len(s)>1:
        for i in range(len(s)):
            replacement_dict[s[i]]=st[i]
...
def recover_tags(trans_bytes:bytes,ori_bytes:bytes)->bytes:
    ...
    #\x81\x40不进行恢复
    voice_tag=re.match(b'[\x00-\xff][\x00-\xff][\x00-\xff][\x01-\x04]',ori_bytes)
...
with open('release\EXDN.SD','wb') as outf:
    outf.write(data)
