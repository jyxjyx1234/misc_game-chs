...
class OriJsonOutput():
    ...
    def move_msg_to_name(self):
        if len(self.dic["idx"]) != 1:
            print(self.dic)
            raise ValueError("remove_msg_to_name can only be used when idx has one element")
        self.dic["name"] = self.dic["message"]
        del self.dic["message"]
        self.dic["name_idx"] = self.dic["idx"].copy()
        del self.dic["idx"]
    ...
    def append_dict(self, quchong = False, remove_name = True):
        if "message" not in self.dic or self.dic['message'] == "":
            # self.dic = {}
            return
        
        if not self.savefilter(self.dic):
            _ = self.dic.copy()
            self.dic = {}
            if "name" in _:
                self.dic['name'] = _["name"]
                self.dic['name_idx'] = _["name_idx"]
            return
        
        if "name" in self.dic:
            if self.dic["name"] == "" or self.dic["name"] == "\u3000":
                del self.dic["name"]
                del self.dic["name_idx"]
        else:
            self.dic["isopt"] = True
...
class StatusInfo:
    ...
    def output(self, isSaveName = False):
