...
将补丁中的内容覆盖到游戏目录。从`NAO2CHS.exe`启动。无需转区。路径不能有中文。

所提供字体无需安装。如果需要使用自定义字体，请将字体文件放入游戏目录，然后在hook.ini中的对应区域修改对应的字体文件名和字体名。字体名和字体文件名不能有非英文。（关于如何查看和修改字体名，请自行百度）更换字体可能导致游戏在某些位置崩溃，由此引入的问题不提供支持。
