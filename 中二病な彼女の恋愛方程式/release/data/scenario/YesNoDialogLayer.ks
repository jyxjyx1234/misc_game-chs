...
class ImageDialogLayerPlugin extends KAGPlugin {
	...
	var replacefuncary1 = [ // プラグイン開始時までに登録する関数郡
		...
		"goJumpCheck",        "goJumpCheck_2nd",
		"goToSaveWithAsk",        "goToSaveWithAsk_2nd",
		"goToLoadWithAsk",		"goToLoadWithAsk_2nd",
		"goToQSaveWithAsk",        "goToQSaveWithAsk_2nd",
		"goToQLoadWithAsk",        "goToQLoadWithAsk_2nd",
	...
	var options = %[
		...
		image    : "YesNoImageDialog_YesNoDialog", 	// YesNoダイアログの画像
		...
		yesbutton : %[			// 「はい」ボタン
			...
			image  : "YesNoImageDialog_YesButton",	// 画像
			left   : 470,		// 表示位置
			top    : 417
		],
		nobutton : %[			// 「いいえ」ボタン
			button : void,		// ボタンインスタンス
			image  : "YesNoImageDialog_NoButton",// 画像
			left   : 672,		// 表示位置
			top    : 417
		],
		okbutton : %[			// 「はい」ボタン
...
// ここ以下はwin(=kag)の関数置き換え用
	...
	function onCloseQuery()	// incontextof win
	{
		...
		// 質問ダイアログを表示してすぐ終わる
		if( sf.exitAsk==1 ){
		obj.askYesNoLayer("要退出吗？", onCloseQuery_2nd);
	...
	// 「シーンジャンプさせるか」
	function goJumpCheck() // incontextof win
	{
		var obj = global.ImageDialogLayerPlugin_obj;
		// 質問ダイアログを表示してすぐ終わる
		obj.askYesNoLayer("要进行场景跳转吗？", goJumpCheck_2nd);
	}
	function goJumpCheck_2nd() // incontextof win
	{
		tf.ScemeJumping=1;

		
		f.SceneJump_Count_SelectNo = -1;
		for(var loop=0;loop<tf.SceneMode_Count_ListMax;loop++)
		{
			dm("シーンジャンプ検索="+sf.SceneJump_List[loop][0]+"　&　"+f.SceneJump_Selectfile);
			dm("シーンジャンプ検索="+sf.SceneJump_List[loop][1]+"　&　"+f.SceneJump_Selecttarget);

			if( sf.SceneJump_List[loop][0]==f.SceneJump_Selectfile&&sf.SceneJump_List[loop][1]==f.SceneJump_Selecttarget )
			{
				dm("発見="+loop);
				...
				{
					f.SceneJump_Count_SelectNo = loop;
		...
		if( f.SceneJump_Count_SelectNo==-1 ){
			.ImageDialogLayerPlugin_obj.dispOKLayer("因未达到条件，无法跳转");
			tf.ScemeJumping=0;
			return;
		}
		if( sf.SceneJump_List[f.SceneJump_Count_SelectNo]===void ){
			.ImageDialogLayerPlugin_obj.dispOKLayer("指定编号的场景跳转数组未正常初始化");
			tf.ScemeJumping=0;
			return;
		}
		tf.file=&sf.SceneJump_List[&f.SceneJump_Count_SelectNo][0];
		tf.target=&sf.SceneJump_List[&f.SceneJump_Count_SelectNo][1];
...
//			[endhact]
		...
		tf.playcvflg=0;
		bgm.fadeOut(%["time"=>"300"]);
		se[0].fadeOut(%["time"=>"300","buf"=>"0"]);
		se[1].fadeOut(%["time"=>"300","buf"=>"1"]);
		
		updateBeforeCh = 1;
		
		stopAllTransitions();
		backupLayer(%["layer"=>"0"], true);

		getMessageLayerObjectFromElm(%["layer"=>"message4","page"=>"fore"]).setPosition(%["visible"=>"false","opacity"=>"0"]);
		getMessageLayerObjectFromElm(%["layer"=>"message4","page"=>"back"]).setPosition(%["visible"=>"false","opacity"=>"0"]);
		getLayerFromElm(%["layer"=>"0","page"=>"back"]).freeImage(%["layer"=>"0","page"=>"back"]);
		getLayerFromElm(%["layer"=>"1","page"=>"back"]).freeImage(%["layer"=>"1","page"=>"back"]);
		getLayerFromElm(%["layer"=>"2","page"=>"back"]).freeImage(%["layer"=>"2","page"=>"back"]);
		getLayerFromElm(%["layer"=>"3","page"=>"back"]).freeImage(%["layer"=>"3","page"=>"back"]);
		getLayerFromElm(%["layer"=>"4","page"=>"back"]).freeImage(%["layer"=>"4","page"=>"back"]);
		getLayerFromElm(%["layer"=>"5","page"=>"back"]).freeImage(%["layer"=>"5","page"=>"back"]);
		getLayerFromElm(%["layer"=>"6","page"=>"back"]).freeImage(%["layer"=>"6","page"=>"back"]);
		getLayerFromElm(%["layer"=>"7","page"=>"back"]).freeImage(%["layer"=>"7","page"=>"back"]);
		getLayerFromElm(%["layer"=>"8","page"=>"back"]).freeImage(%["layer"=>"8","page"=>"back"]);
		getLayerFromElm(%["layer"=>"9","page"=>"back"]).freeImage(%["layer"=>"9","page"=>"back"]);
		getLayerFromElm(%["layer"=>"10","page"=>"back"]).freeImage(%["layer"=>"10","page"=>"back"]);

		getLayerPageFromElm(%["layer"=>"base","method"=>"crossfade","time"=>"1000"], false).beginTransition(%["layer"=>"base","method"=>"crossfade","time"=>"1000"]);
		stopAllTransitions();
//		waitTransition(%["method"=>"10","crossfade"=>"1000"]);
		//全キャラ消去
		for(var i = chardatas_obj.names.count-1; i >= 0; i--)
		{
			chardatas_obj.delChar(chardatas_obj.names[i]);
		}
		
		
		tf.Scenejumptimer.enabled = true;
	}
	...
	function goBackHistory(ask = true) // incontextof win
	{
		...
		var prompt = "要返回到「"+ historyOfStore[0].core.currentPageName + "」吗?";
	...
	function saveBookMarkWithAsk(num) // incontextof win
	{
		...
		if(num < numBookMarks)
			prompt += (num + 1);
		if(bookMarkDates[num] != "") // 空字符的情况下存档不存在
			prompt += "「" + bookMarkNames[num] + "」";
		prompt += "处保存「"+ pcflags.currentPageName + "」吗?";
	...
	function goToQLoadWithAsk()
	{
		...
		f.SCutEnableFlg = 0;
		tf.SaveMode_Flag_Mode=0;
		sf.Quicl_Flag_Mode=1;
		tf.return=1;
		if( sf.ConfigState17 == 1 ){
			var obj = global.ImageDialogLayerPlugin_obj;
			var prompt = "要快速读取吗？";
			obj.askYesNoLayer(prompt, goToQLoadWithAsk_2nd);
		}
		else{
			goToQLoadWithAsk_2nd();
		}


	}
	function goToQLoadWithAsk_2nd()
	{
		tf.SaveMode_Flag_ViewCount = tf.LoadIndex;
		
		
		SysBtn_LoadFunction(tf.LoadIndex);
		
		//クイックロード処理抜け
		f.SCutEnableFlg = 1;
		sf.Quicl_Flag_Mode = 0;

//		if( tf.OpenQucikDrawFlag == 1 )
//			SysBtn_CreateRightMenuButton(1);
	}

	//クイックセーブ実行
	function goToQSaveWithAsk()
	{
		//クイックセーブの為にスナップショットを作成
		//-右メニューは消去される
		SysBtn_CreateRightMenuButton(0);
		...
		if( sf.ConfigState16 == 1 ){
			var obj = global.ImageDialogLayerPlugin_obj;
			var prompt = "要快速保存吗？";
			obj.askYesNoLayer(prompt, goToQSaveWithAsk_2nd);
		}
		else{
			goToQSaveWithAsk_2nd();
		}
	}
	function goToQSaveWithAsk_2nd()
	{
		tf.SaveMode_Flag_ViewCount = tf.saveindex;
		
		SysBtn_SaveFunction(tf.saveindex);
		
		
		//Qセーブされたサムネイルを表示
		if( tf.OpenQucikDrawFlag == 1 )
			SysBtn_QuickSaveSlotSelect(tf.saveindex-90);
	...
	function goToSaveWithAsk()
	{
		if( tf.saveindex < 90 )
		{
			if( sf.ConfigState14==1 )
			{
				var obj = global.ImageDialogLayerPlugin_obj;
				var prompt = "要保存吗？";
				obj.askYesNoLayer(prompt, goToSaveWithAsk_2nd);
	...
	function goToSaveWithAsk_2nd()
	{
		//セーブ中フラグをONにする。セーブ中は新たなセーブができない
		tf.saveingnow = 1;
		
		//セーブ実行
		var elm = %["ask"=>void,"place"=>tf.saveindex];
		if(elm.ask !== void && +elm.ask)
			kag.saveBookMarkWithAsk(+elm.place);
		else
			kag.saveBookMark(+elm.place);
		
		sf.SaveMode_Flag[tf.saveindex][0]=1;
		
		dm("実験="+kag.fore.messages[2].lastDrawnCh);
		
		if( kag.RunSaveLoadModeFlg == 1){
			//セーブロード画面起動中の場合はサムネイルリフレッシュ
			SysBtn_RefleshSaveLoadPage();//ページをリフレッシュ
			SysBtn_RefleshSaveLoadThum();//サムネイルをリフレッシュ
		}
		tf.saveingnow = 0;

	}
	//ロード実行
	function goToLoadWithAsk()
	{
		if( tf.LoadIndex < 90 )
		{
			if( sf.ConfigState15==1 )
			{
				var obj = global.ImageDialogLayerPlugin_obj;
				var prompt = "要读取吗？";
				obj.askYesNoLayer(prompt, goToLoadWithAsk_2nd);
	...
	function goToLoadWithAsk_2nd()
	{
		...
		//ロード実行
		var elm = %["ask"=>void,"place"=>tf.LoadIndex];
		if(elm.ask !== void && +elm.ask)
			kag.loadBookMarkWithAsk(+elm.place);
		else
			kag.loadBookMark(+elm.place);

		sf.QSaveLoadSelectIndex = -1;
		kag.RunSaveLoadModeFlg = false;
		sf.SaveMode_Flag[tf.LoadIndex][0]=1;
		
		
		//ボリューム値を再設定
		kag.bgm.setOptions(%["gvolume"=>sf.Config_Volume1,"volume"=>sf.Config_Volume2]);
		kag.se[0].setOptions(%["gvolume"=>sf.Config_Volume1,"volume"=>sf.Config_Volume3]);
		kag.se[1].setOptions(%["gvolume"=>sf.Config_Volume1,"volume"=>sf.Config_Volume4]);
