	...
	var tagFuncs = %[
		...
		hbutton: %[
			drawfunc: function(e, x, y, top_x, top_y, dx, dy) {
				...
				var hb = new HistoryButtonLayer(o.window, o);
				hbuttons.add(hb);
				e.graphic = "image/history_voice2.png";
				dm("graphic"+e.graphic);
...
//	var fontface;			// 現在のフォント名
	...
	var hbCaptionColor = 0x000000;	// 履歴ボタンキャプション色
...
class HistoryActionButtonLayer extends ButtonLayer
{
	...
	function setSize(w, h)
	{
		...
		fillRect(w,  0, w,h, 0x00000000|c);	// 押された時
		fillRect(w*2,0, w,h, 0x00000000|c);	// フォーカス1
